...
class Anuncio extends BaseController
{
    ...
    public function misAnuncios($page = 1){
        ...
        $data['title']        = 'Mis Anuncios | '.help_nombreWeb();
        $data['opt_anuncios'] = 1;
    ...
    public function publicarAnuncio($idanuncio = ''){
        ...
        $data['title'] = 'Nuevo Anuncio | '. help_nombreWeb();
    ...
    public function comprarAnuncios(){
        if( !session('idusuario') ){
            return redirect()->to('ingresar');
        }
        ...
        $data['title'] = 'Comprar Anuncios | '.help_nombreWeb();

        $listPrecios = $this->modeloAnuncio->listPrecios('anu');
        $data['preciosAnuncios'] = $listPrecios;

        return view('panel/usuario/compra_anuncio', $data);
