...
class AnuncioModel extends Model{
    ...
    public function getAnu_idanu($idanuncio, $status = [1,2,3,4,5,6,7]){
        ...
        left join destacado des on anu.iddestacado=des.iddestacado
        where anu.idanuncio = ? and anu.an_status in ? and img.principal = 1";
        $st = $this->db->query($query, [$idanuncio, $status]);
    ...
    public function busqueda($desde, $hasta, $criterios = '', $params = [], $keyword = '', $order = '', $status = [2,4,5]){
        ...
        $query = "select anu.idanuncio, anu.an_nombre, anu.idcate, anu.precio, anu.precio_mostrar, anu.direccion,
    ...
    public function getFavoritos_x_Usuario($idusuario){
        ...
        anu.an_nombre, anu.idcate, anu.an_status, anu.codanuncio,
        can.categoria,
        img.idimages, img.img, img.img_thumb
        from favorito fav
